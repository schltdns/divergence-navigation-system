...
\usepackage[hidelinks,
            pdftitle={DNS -- Divergence-based Navigation System},
...
% ── Metadata ──────────────────────────────────────────────────────────────────
\title{\textbf{DNS — Divergence-based Navigation System}:\\
...
\author{Denis Schult\\
        ...
        \small Version 3.8.2-DNS (Open) \quad April 2026}
...
We introduce the \textit{Divergence-based Navigation System (DNS)}, a structured,
prompt-level multi-agent protocol that treats divergence—contradiction, refusal, and
...
falsification thresholds, role-based model selection (Team Architecture v1.5),
parallel elicitation, divergence mapping and typing, weighted human synthesis,
external validation, operator reflection, and versioned archiving.
...
Case Study~2 (German labour market 2030, six models—Grok, ChatGPT, Gemini,
Copilot, Claude, DeepSeek—following the DNS Team Architecture v1.5) produces a
divergence-structured synthesis across seven analytical dimensions (A–G), including
testable falsification criteria with explicit thresholds (e.g., EU productivity
growth $<0.1\%$~p.a.\ by 2028 would falsify the optimistic productivity thesis)
and a power-layer audit of each core claim.

\noindent\textbf{Conclusion.}
DNS is a heuristic, human-centered framework for reflective decision-making under
...
The \textit{Divergence-based Navigation System (DNS)}\footnote{The term
``triangulation'' is used in the sociological tradition of \citet{denzin1970} and
\citet{patton1999}, where multiple data sources are used not to confirm but to
reveal discrepancies. DNS extends this logic to LLM outputs while foregrounding
structured divergence over consensus.}
...
DNS differs by foregrounding divergence rather than reducing it, and by using LLMs
...
\citep{breiman1996}. They optimize for agreement; DNS optimizes for structured
...
outputs can improve factual accuracy. Their protocols aim for resolution; DNS
preserves and documents unresolved divergence.

\subsection{Red Teaming}
\citet{Perez2022} probe single models adversarially to surface vulnerabilities. DNS
...
properties of model outputs. DNS is qualitative and role-based, foregrounding
...
solve a technical coordination problem. DNS operates at a higher logical level: it
...
Where orchestration frameworks seek efficiency and task completion, DNS seeks
...
alternative perspectives that cannot be immediately resolved. DNS operationalizes
...
\subsection{Team Architecture v1.5}
DNS utilises a specialised 7-role matrix. Each node is a functional organ of the
navigation system.

\begin{table}[h]
\centering
\caption{DNS Team Architecture v1.5 – functional roles and instances.}
\label{tab:team}
\begin{tabularx}{\textwidth}{lXl}
\toprule
\textbf{Role} & \textbf{Instance} & \textbf{Function} \\
\midrule
S1: Kinetic-Agent       & Grok     & Real-time OSINT, X-signal pulses, anomaly detection \\
S2: Structural-Architect & ChatGPT & Framework integrity, documentation, structural logic \\
S3: Synthesis-Bridge    & Gemini   & Strategic vision, context expansion, scenario building \\
S4a: Visualizer         & Copilot  & Source matrix, data synthesis, operational tooling \\
S4b: Boundary-Marker    & Claude   & Epistemic precision, risk boundaries, safety logic \\
$\Omega$: Falsificator  & DeepSeek & Logical stress testing, falsification, backtesting \\
P5/6: Operator          & Human    & Final synthesis, normative decision, meta-reflection \\
\bottomrule
\end{tabularx}
\end{table}

Roles are \textit{illustrative, not fixed} – they can be reassigned to other models
with similar epistemic profiles. The independence is functional, not architectural.

\subsection{Divergence Typology}
\label{sec:typology}

DNS classifies divergence along four axes:

\begin{table}[h]
\centering
\caption{DNS divergence typology with handling rules.}
\label{tab:typology}
\begin{tabularx}{\textwidth}{lXl}
\toprule
...
For applied case studies, DNS introduces a three-filter model to structure how
...
This model operationalises the \textit{master question} of applied DNS studies:
\textit{Does the technology scale faster than social systems can adapt?}

\subsection{The Power Layer (P6b)}
\label{sec:powerlayer}

The three filters are not power-neutral. For every core synthesis claim, DNS requires
...
\node[box, below=of p1] (p2) {P2 — Role-Based Model Selection (Team v1.5)};
...
\end{tikzpicture}
\caption{The eight-phase DNS cycle. Dashed arrows indicate the power-layer
...
Assign available models to functional roles according to the Team Architecture
v1.5 (Table~\ref{tab:team}). Roles are descriptive, not prescriptive.
...
with an identical prompt analysing TTF gas prices and German industrial resilience
...
The refusal-as-signal treatment demonstrates a core DNS design choice: model
...
precisely the conditions DNS is designed to handle: genuine empirical uncertainty,
strong normative disagreement, and structural power asymmetries.

\textbf{Models (Team Architecture v1.5):} Grok (S1), ChatGPT (S2), Gemini (S3),
Copilot (S4a), Claude (S4b), DeepSeek ($\Omega$).\\
...
The DNS analysis produces the following master frame for the German labour
market 2030:
...
\subsection{What DNS Adds to Existing Methods}
Standard LLM use cases produce a single synthesized answer. Multi-agent debate
protocols produce a corrected answer. DNS produces a \textit{structured map of
...
\textbf{Educational context.} DNS's explicit treatment of normative divergence
...
\subsection{DNS as a Benchmark for Model Behaviour}
...
observable: US-trained models (Grok, ChatGPT) tended toward market-optimistic
...
training-data differences. The DNS framework offers a replicable protocol for
...
\begin{itemize}
  \item \textbf{Operator dependence.} The quality of the synthesis is bounded by
        ...
        DNS does not eliminate this dependence; it makes it explicit and auditable.
  ...
  \item \textbf{No quantification.} DNS does not produce probability estimates or
...
\textit{The Divergence-based Navigation System (DNS)} is a structured, human-centered
protocol for epistemic divergence analysis across LLMs. Its core contribution is the
reframing of model disagreement—including refusals, normative conflicts, and
quantitative discrepancies—as informative signals rather than noise.
...
as authoritative, DNS offers a countervailing discipline: \textit{show the
...
Study~2—Grok (xAI), ChatGPT (OpenAI), Gemini (Google), Copilot (Microsoft),
Claude (Anthropic), and DeepSeek—whose systematic disagreements provided the
